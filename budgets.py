...
class BudgetTracker:
    def __init__(self, limits: BudgetLimits | None = None):
        ...
        self.turns = self.tokens = self.calls = 0
        self.cost_usd = 0.0
    ...
    def check(self):
        limits = self.limits
        if (
            limits.max_duration is not None
            and time.monotonic() - self.started > limits.max_duration
        ):
            raise BudgetExceeded("Durée maximale dépassée")
        if limits.max_turns is not None and self.turns >= limits.max_turns:
            raise BudgetExceeded("Nombre maximal de tours dépassé")
        if limits.max_tokens is not None and self.tokens >= limits.max_tokens:
            raise BudgetExceeded("Budget de tokens dépassé")
        if limits.max_cost_usd is not None and self.cost_usd >= limits.max_cost_usd:
            raise BudgetExceeded("Budget coût dépassé")
        if limits.max_calls is not None and self.calls >= limits.max_calls:
            raise BudgetExceeded("Nombre maximal d'appels dépassé")
...
class CircuitBreaker:
    def __init__(self, threshold=5, recovery=30.0):
    ...
    def allow(self):
        if self.opened and time.monotonic() - self.opened < self.recovery:
            return False
        return True
