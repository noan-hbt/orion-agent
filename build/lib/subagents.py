...
class SubAgentManager:
    ...
    def _save_locked(self) -> None:
        ...
        published = sorted(
            (item for item in self._outbox.values() if item.get("published")),
        ...
        retained_outbox.update(
            (str(key), item) for key, item in self._outbox.items() if not item.get("published")
        )
    ...
    def _drain_outbox(self) -> None:
        ...
        with self._lock:
            ...
            for item in self._outbox.values():
                ...
                if not item.get("published") and key and key not in self._outbox_claimed:
