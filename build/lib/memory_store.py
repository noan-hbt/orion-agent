...
class MemoryStore:
    def __init__(self, path: str = ":memory:", *, max_items: int = 10000, max_content_chars: int = 20000):
        if max_items < 1 or max_content_chars < 1: raise ValueError("memory limits must be positive")
    ...
    def _purge(self) -> None:
        self._db.execute("DELETE FROM memories WHERE expires_at IS NOT NULL AND expires_at <= ?", (time.time(),))
        self._db.commit()
    ...
    def put(self, content: str, *, namespace: str = "default", provenance: str | None = None,
        ...
        if not namespace or len(content) > self.max_content_chars: raise ValueError("invalid memory namespace or content size")
        if not 0 <= float(confidence) <= 1: raise ValueError("confidence must be between 0 and 1")
        if not 0 <= float(freshness) <= 1: raise ValueError("freshness must be between 0 and 1")
        if status not in {"active", "tombstone", "superseded", "retracted"}:
            raise ValueError("invalid memory status")
        now = time.time(); ident = item_id or uuid.uuid4().hex; expires = now + float(ttl) if ttl is not None else None
        ...
        self._db.execute("DELETE FROM memories WHERE namespace=? AND id NOT IN (SELECT id FROM memories WHERE namespace=? ORDER BY updated_at DESC LIMIT ?)", (namespace, namespace, self.max_items))
        self._db.commit(); return self.get(ident, namespace=namespace)  # type: ignore[return-value]

    def get(self, item_id: str, *, namespace: str = "default") -> MemoryItem | None:
        self._purge(); row = self._db.execute("SELECT * FROM memories WHERE id=? AND namespace=?", (item_id, namespace)).fetchone()
    ...
    def search(self, query: str = "", *, namespace: str = "default", limit: int = 20,
        ...
        """
        self._purge(); limit = max(1, min(int(limit), 100)); args: list[Any] = [namespace]
        min_freshness = float(min_freshness)
        if not 0 <= min_freshness <= 1: raise ValueError("min_freshness must be between 0 and 1")
        sql = "SELECT * FROM memories WHERE namespace=? AND status='active' AND freshness>=?"; args.append(min_freshness)
        if not include_without_consent: sql += " AND consent=1"
        if scope is not None: sql += " AND scope=?"; args.append(str(scope))
    ...
    def forget(self, item_id: str, *, namespace: str = "default", tombstone: bool = False) -> bool:
        if tombstone:
            cur = self._db.execute("UPDATE memories SET status='tombstone', updated_at=? WHERE id=? AND namespace=?", (time.time(), item_id, namespace))
        else:
            cur = self._db.execute("DELETE FROM memories WHERE id=? AND namespace=?", (item_id, namespace))
        self._db.commit(); return cur.rowcount > 0
