...
class Service:
    ...
    def create_agent(self, **kw): self.calls.append(("spawn", kw)); return {"id": "a1"}
    def delete_agent(self, agent_id): self.calls.append(("kill", agent_id)); return {"id": agent_id}
    def submit(self, objective, *, agent_id=None, **kw): self.calls.append(("delegate", agent_id, objective, kw)); return {"id": "j1"}
    def send_message(self, job_id, message, *, caller_scope=None): self.calls.append(("send", job_id, message)); return {"id": job_id}
...
def test_subagent_actions_are_explicit_and_serialized():
    ...
    assert execute_action(app, "delegate", {"agent_id": "a1", "objective": "inspect"})["ok"]
