...
class Outbox:
    ...
    def __init__(
        self,
        path: str | Path = "data/outbox.sqlite3",
        *,
        clock=time.time,
        ...
        self.clock, self.max_backoff, self.base_backoff, self.jitter = (
            clock,
            max_backoff,
            base_backoff,
            jitter,
        ...
        """)
        try:
            self._db.execute("ALTER TABLE outbox ADD COLUMN lease_owner TEXT")
    ...
    def enqueue(
        ...
        *,
        ...
        row = self._db.execute(
            "SELECT id,channel,payload FROM outbox WHERE idempotency_key=?", (key,)
        ).fetchone()
        ...
        *,
        ...
        try:
            ...
            rows = self._db.execute(
                f"SELECT * FROM outbox WHERE {where} ORDER BY id LIMIT ?",
                (*args, max(0, limit)),
            ).fetchall()
            until = now + max(0.1, lease_seconds)
            for row in rows:
                self._db.execute(
                    "UPDATE outbox SET state='sending', lease_until=?, lease_owner=?, attempts=attempts+1 WHERE id=?",
                    (until, worker_id, row["id"]),
                )
    ...
    def ack(self, item_id: int, worker_id: str | None = None) -> bool:
        now = self.clock()
        cur = self._db.execute(
            "UPDATE outbox SET state='sent', lease_until=NULL, lease_owner=NULL, sent_at=? WHERE id=? AND state='sending' AND lease_until>? AND ((? IS NULL AND lease_owner IS NULL) OR (? IS NOT NULL AND lease_owner=?))",
            (now, item_id, now, worker_id, worker_id, worker_id),
        )
    ...
    def fail(
        ...
        *,
        ...
        now = self.clock()
        row = self._db.execute(
            "SELECT attempts,lease_owner,lease_until FROM outbox WHERE id=? AND state='sending'",
            (item_id,),
        ).fetchone()
        if not row:
            return False
        ...
        cur = self._db.execute(
            "UPDATE outbox SET state=?,available_at=?,lease_until=NULL,lease_owner=NULL,last_error=? WHERE id=? AND state='sending' AND lease_until>? AND ((? IS NULL AND lease_owner IS NULL) OR (? IS NOT NULL AND lease_owner=?))",
            (
                state,
                when,
                str(error)[:4000],
    ...
    def renew(
        self, item_id: int, lease_seconds: float = 60.0, worker_id: str | None = None
    ) -> bool:
        now = self.clock()
        cur = self._db.execute(
            "UPDATE outbox SET lease_until=? WHERE id=? AND state='sending' AND lease_until>? AND ((? IS NULL AND lease_owner IS NULL) OR (? IS NOT NULL AND lease_owner=?))",
            (
                now + max(0.1, lease_seconds),
                item_id,
                now,
                worker_id,
                worker_id,
    ...
    def release(self, item_id: int, worker_id: str | None = None) -> bool:
        now = self.clock()
        cur = self._db.execute(
            "UPDATE outbox SET state='pending', lease_until=NULL, lease_owner=NULL, available_at=? WHERE id=? AND state='sending' AND lease_until>? AND ((? IS NULL AND lease_owner IS NULL) OR (? IS NOT NULL AND lease_owner=?))",
            (now, item_id, now, worker_id, worker_id, worker_id),
        )
        return bool(cur.rowcount)
    ...
    def replay(self, *, include_failed: bool = True, include_sent: bool = False) -> int:
        ...
        """
        states = (("sent",) if include_sent else ()) + (
            ("failed",) if include_failed else ()
        )
        if not states:
            return 0
    ...
    def _item(self, r: sqlite3.Row, **overrides: Any) -> OutboxItem:
        return OutboxItem(
            ...
            r["idempotency_key"],
            overrides.get("state", r["state"]),
            overrides.get("attempts", r["attempts"]),
            r["available_at"],
            overrides.get("lease_until", r["lease_until"]),
            r["last_error"],
            overrides.get("lease_owner", r["lease_owner"]),
        )
