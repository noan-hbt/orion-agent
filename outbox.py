...
class Outbox:
    ...
    def fail(self, item_id: int, error: str, *, retry: bool = True, retry_at: float | None = None, worker_id: str | None = None) -> bool:
        ...
        if worker_id is not None and row["lease_owner"] != worker_id: return False
        if retry:
        ...
        cur = self._db.execute("UPDATE outbox SET state=?,available_at=?,lease_until=NULL,lease_owner=NULL,last_error=? WHERE id=? AND state='sending' AND lease_until>? AND (? IS NULL OR lease_owner=?)", (state, when, str(error)[:4000], item_id, now, worker_id, worker_id))
        return bool(cur.rowcount)

    def renew(self, item_id: int, lease_seconds: float = 60.0, worker_id: str | None = None) -> bool:
        now=self.clock(); cur = self._db.execute("UPDATE outbox SET lease_until=? WHERE id=? AND state='sending' AND lease_until>? AND (? IS NULL OR lease_owner=?)", (now + max(.1, lease_seconds), item_id, now, worker_id, worker_id))
        return bool(cur.rowcount)

    def release(self, item_id: int, worker_id: str | None = None) -> bool:
        now=self.clock(); cur = self._db.execute("UPDATE outbox SET state='pending', lease_until=NULL, lease_owner=NULL, available_at=? WHERE id=? AND state='sending' AND lease_until>? AND (? IS NULL OR lease_owner=?)", (now, item_id, now, worker_id, worker_id))
        return bool(cur.rowcount)
    ...
    def replay(self, *, include_failed: bool = True, include_sent: bool = False) -> int:
        ...
        cur = self._db.execute(f"UPDATE outbox SET state='pending', available_at=?, lease_until=NULL WHERE state IN ({marks})", (self.clock(), *states))
        return cur.rowcount
