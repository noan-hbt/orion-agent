...
def execute_action(application: Any, command: str, args: Any = None) -> dict[str, Any]:
    ...
    try:
        ...
        if name == "spawn":
            ...
            data = manager.create_agent(name=str(val("name", 0, val("role", 0, "agent"))), description=str(val("description", 1, "")), model=val("model", 2), system_prompt=str(val("system_prompt", 3, "")), allowed_tools=list(val("allowed_tools", default=[])), capabilities=list(val("capabilities", default=[])), max_turns=int(val("max_turns", default=8)))
        elif name == "kill":
            ...
            else: return _result(name, error="Service de communication indisponible")
        elif name == "approve":
            if approvals is None or not callable(getattr(approvals, "approve", None)):
                return _result(name, error="Service approvals indisponible")
            data = approvals.approve(str(val("approval_id", 0)), decided_by=str(val("decided_by", default="user")), reason=val("reason", default=None))
        elif name in {"autonomy", "workspace"}:
            target = val(name, 0)
            setter = _get(application, f"set_{name}")
            if not callable(setter): return _result(name, error=f"Aucun setter {name} disponible")
            data = setter(target)
        else: return _result(name, error=f"Action inconnue: /{name}")
